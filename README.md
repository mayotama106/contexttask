...
`Tagger` インターフェース (`src/features/capture/aiTagger.ts`) の背後にある。現在の実装 `HeuristicTagger` はキーワード照合のローカルスタブなので、モデル呼び出しに差し替えればよい。キューの性質:
...
**注意:** キャプチャは `!期限` があると `important: true` にする（ハンドオフ仕様）。そのため「重要」は実質「日付があるもの全部」になる。今日画面の「重要」グループが未来の予定で埋まらないよう、そこは日付なしのものだけを出している。仕様を変えるなら要相談。
